...
KINESIS_ITEM_STREAM = "muravev-capstone-views"
KINESIS_REVIEW_STREAM = "muravev-capstone-reviews"
...
def generate_batch(users, items):
    ...
    with io.StringIO() as batch:
        ...
        boto3.client("s3").upload_fileobj(
            io.BytesIO(batch.getvalue().encode()),
            S3_BUCKET,
            S3_PREFIX + prev_hour.strftime("/%Y/%m/%d/%H.csv")
        )
